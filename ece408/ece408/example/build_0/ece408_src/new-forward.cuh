...
__global__ void forward_kernel(DType *y1, const DType *x1, DType *y2, const DType *x2, const int B, const int ii) {
    ...
    // }
    // second image
    // check if second image is needed
    if(ii>0 && bx + (B-1)/2+1 > B)
        return;
