...
texture<float, cudaTextureType2D, cudaReadModeElementType> images;
...
__global__ void forward_kernel(DType *y) {
    ...
    for(int b=blockIdx.x; b<10000; b+=batch_size){
        for(int m=0; m<50; m++){
            for(int j=0; j<K; j++)
                for(int i=0; i<K; i++)
                    acc += float(tex2D(images, b*input_size+(t_y+j)*28, (t_x+i))) * filters[m*25 + j*5 +i];
            y[b*TOTAL + m*output_size + t_y*24 + t_x] = acc;
            acc =0;
        }
    }
}
