...
__global__ void forward_kernel(DType *y, const DType *x) {
    ...
    float acc;
    int counter, b, m, i, j, t_x = threadIdx.x%24, t_y = (threadIdx.x>>3)/3, b_x = blockIdx.x;

    for(b=b_x, counter=0; b<10000; b+=batch_size, counter++){
        for(i=t_y; i<28; i+=TILE_WIDTH){
            for(j=t_x; j<28; j+=TILE_WIDTH)
                X_shared[counter*input_size+ (i<<2)*7 + j] = x[b*input_size + (i<<2)*7 + j];
        }
    }
    ...
    for(b=b_x, counter=0; b<10000; b+=batch_size, counter++){
        for(m=0; m<50; m++){
            ...
            for(j=0; j<K; j++){
                for(i=0; i<K; i++)
                        acc += X_shared[counter*input_size + ((t_y+j)<<2)*7 + t_x+i] * filters[m*25 + j*K + i];
            }
...
void forward(mshadow::Tensor<gpu, 4, DType> &y, const mshadow::Tensor<gpu, 4, DType> &x, const mshadow::Tensor<gpu, 4, DType> &k) {
    ...
    forward_kernel<gpu, DType><<<batch_size, output_size, sizeof(float)*(input_size*(B/batch_size)), y.stream_->stream_>>>(y.dptr_,x.dptr_);
