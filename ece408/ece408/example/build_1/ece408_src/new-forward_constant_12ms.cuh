...
__global__ void forward_kernel(DType *y, const DType *x, const DType *k) {
    ...
    #define x4d(i3,i2,i1,i0) x[(i3) * (IN_SIZE * IN_SIZE) + (i2)*(IN_SIZE * IN_SIZE) + (i1)*(IN_SIZE) + i0]
    ...
    for (int i = h; i < IN_SIZE; i += BLOCK_SIZE) {
        for (int j = w; j < IN_SIZE; j += BLOCK_SIZE) {
            if (i < IN_SIZE && j < IN_SIZE) {
                x_shared[i * IN_SIZE + j] = x4d(b, 0, i, j);
            } else {
                x_shared[i * IN_SIZE + j] = 0;
            }
        }
    }
    __syncthreads();
