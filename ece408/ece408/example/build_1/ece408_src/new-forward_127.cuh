...
__global__ void forward_kernel(DType *y, const DType *x, const DType *k, const int B, const int M, const int C, const int H, const int W, const int K) {
    ...
    DType result = 0;
    for (int c = 0; c < C; c++) {
    	// loading data from global w to w_shared
    	/*
    	for (int i = threadIdx.y; i < K; i += BLOCK_SIZE) {
    		for (int j = threadIdx.x; j < K; j += BLOCK_SIZE) {
    	...
    	*/
    	...
    	// loading data from global x to x_shared
