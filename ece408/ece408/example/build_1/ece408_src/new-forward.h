...
void forward(mshadow::Tensor<cpu, 4, DType> &y, const mshadow::Tensor<cpu, 4, DType> &x, const mshadow::Tensor<cpu, 4, DType> &k)
{
    ...
    */
    ...
    for (int b = 0; b < B; ++b) {   // for each image in the batch, 10000
        for (int m = 0; m < M; ++m) {   // for each output feature maps
            for (int h = 0;  h < H; ++h) {  // for each output element, 28x28
                for (int w = 0; w < W; ++w) {
                    y[b][m][h][w] = 0;
                    for (int c = 0; c < C; ++c) {   // sum over all input feature maps, 1
                        for (int p = 0; p < K; ++p) {   // KxK filter, 5x5
                            for (int q = 0; q < K; ++q) {
