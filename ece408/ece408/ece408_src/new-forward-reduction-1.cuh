...
__global__ void first_forward_kernel(float *y, const float *x)
{
  ...
  n = blockIdx.x;
  m = blockIdx.y;
  h = blockDim.y * (blockIdx.z / ((W_out_first - 1) / BLOCK_SIZE + 1)) + tidx % BLOCK_SIZE;
  w = blockDim.x * (blockIdx.z % ((W_out_first - 1) / BLOCK_SIZE + 1)) + tidx / BLOCK_SIZE;
  ...
  */
  ...
  if(h < H_out_first && w < W_out_first)
  {
    ...
    for(p = 0; p < K; p++)
    {
      S_M[tidx * K * K + p * K+0] = x4d_first(n, 0, h + p, w + 0) * k4d_first(Mask, m, 0, p, 0);
      S_M[tidx * K * K + p * K+1] = x4d_first(n, 0, h + p, w + 1) * k4d_first(Mask, m, 0, p, 1);
      S_M[tidx * K * K + p * K+2] = x4d_first(n, 0, h + p, w + 2) * k4d_first(Mask, m, 0, p, 2);
      S_M[tidx * K * K + p * K+3] = x4d_first(n, 0, h + p, w + 3) * k4d_first(Mask, m, 0, p, 3);
      S_M[tidx * K * K + p * K+4] = x4d_first(n, 0, h + p, w + 4) * k4d_first(Mask, m, 0, p, 4);
    }
   __syncthreads();
...
for(unsigned int i = 0; i < BLOCK_SIZE* BLOCK_SIZE ; i++)
  ...
  for (unsigned int stride = 16; stride >= 1;  stride >>= 1)
    {
      __syncthreads();
      if (tidx < stride && tidx+stride < K*K)
        S_M[tidx+i*(K*K)] += S_M[tidx+i*(K*K) + stride];
    }
