...
static int mmap_simple_test(void){
  ...
  addr32 ptr = syscall_mmap(PAGE_SIZE, PROT_WRITE | PROT_READ,
                            MAP_PRIVANON, NULL, 0);
...
static int mmap_failure_test(void){
  ...
  testcase_assert(ptr == MAP_FAILED,
    "mmap succeeded with illegal combination of MAP_SHARED and MAP_PRIVATE");

  ptr =
    syscall_mmap(PAGE_SIZE, PROT_NONE, MAP_PRIVANON, NULL, 0);
  ...
  ptr =
    syscall_mmap(PAGE_SIZE, PROT_READ, MAP_PRIVANON, NULL, 0);
  testcase_assert(ptr != MAP_FAILED, "mmap failed");
  ...
  ptr =
    syscall_mmap(PAGE_SIZE, PROT_READ, MAP_SHARED, "nonexistent_file", 0);
  testcase_assert(ptr == MAP_FAILED,
                  "mmap succeeded despite being given nonexistent file");
  uint32_t too_big =
    current->stack_segment->end_addr - current->heap_segment->end_addr +
    PAGE_SIZE;
  ptr =
    syscall_mmap( too_big, PROT_READ, MAP_PRIVANON, NULL, 0);
  testcase_assert(ptr == MAP_FAILED,
...
static int mmap_stress_allocation_test(void){
  ...
  for (unsigned i = 0; i < 5; ++i){
    ...
    testcase_assert(ptrs[i] >= brk && ptrs[i] <= stack - sizes[i],
	...
	"Overlap between VMA #%u with boundaries (%#08x, %#08x) and VMA #%u "
	"with boundaries (%#08x, %#08x)", first, second,
	ptrs[first], ptrs[first] + sizes[first],
	ptrs[second], ptrs[second] + sizes[second]);
    }
  }
...
static int sbrk_test(void){
  ...
  addr32 ptr = syscall_mmap(500 * PAGE_SIZE,
                            PROT_WRITE | PROT_READ, MAP_PRIVANON, NULL, 0);
  ...
  testcase_assert(res == FAULT_SEGV, "Write past break did not cause segfault");
  ...
  testcase_assert(!strcmp(buff, "Seg"), "Expected to read \"Seg\", got \"%s\"",
                  buff);
...
static int private_file_swapout_test(void){
  ...
  addr32 ptr =
    syscall_mmap(50 * PAGE_SIZE, PROT_READ | PROT_WRITE,
  ...
  while( (pg = get_next_unused_page(pg) )){
    if (pages_desired){
      accessible_pfns[10 - pages_desired] = page_to_phys(pg) >> NUM_OFFSET_BITS;
      --pages_desired;
    }else{
      pg->reserved = 1;
    }
  ...
  for(size_t i = 0; i < 50 * PAGE_SIZE / sizeof(i); ++i)
  {
    ...
    testcase_assert(res == (should_fault ? FAULT_MAJOR : FAULT_NONE),
      "#%zu: Expected %s fault", i, should_fault ? "major" : "no");
    pte_t *pte = get_pte(current, ptr + sizeof(size_t) * i, false);
    testcase_assert(pte && pte->available, "#%zu: PTE on virtual address not "
                    "available after resolved fault.", i);
    ...
    testcase_assert(should_fault ? res == FAULT_MINOR : res == FAULT_NONE,
  ...
  for(size_t i = 0; i < 50 * PAGE_SIZE / sizeof(i); ++i){
    size_t j = 0;
    testcase_assert(fread(&j, sizeof(size_t), 1, tmpfp) == 1, "fread failed");
  ...
  {
    buff = 0;
    testcase_assert(fwrite(&buff, sizeof(buff), 1, tmpfp) == 1,
...
static int load_shared_file_test(void){
  ...
  addr32 ptr2 = syscall_mmap(PAGE_SIZE, PROT_WRITE | PROT_READ,
                             MAP_SHARED, filename, PAGE_SIZE);
  ...
  ptr1 = syscall_mmap(PAGE_SIZE, PROT_WRITE | PROT_READ,
                             MAP_SHARED, filename, PAGE_SIZE);
...
static int page_out_shared_file_test(void){
  ...
  addr32 proc1_ptr2 = syscall_mmap(10 * PAGE_SIZE, PROT_READ, MAP_SHARED,
                                   filename, 0)
		      + 2 * PAGE_SIZE;
  ...
  testcase_assert(
    !page_out_shared_page(phys_to_page(pte12->base_addr << NUM_OFFSET_BITS)), 
    "Page out failed?");
