...
void to_physical(page_table_entry * pte) {
   addr32 f = ask_kernel_for_frame(pte);
   pte->present = 1;
   pte->accessed = 1;
   pte->read_write = 1;
...
void mmu_read_from_virtual_address(mmu *this, addr32 virtual_address,
    ...
    addr32 addr_1 = virtual_address >> 22;
    addr32 addr_2 = (virtual_address << 10) >> 22;
    addr32 offset = virtual_address & 0xfff;
    page_table_entry *result = tlb_get_pte(&this->tlb, virtual_address>>12);

    if(!result) {
       mmu_tlb_miss(this);
       page_directory * enter = this->page_directories[pid];
       page_directory_entry * pde = &enter->entries[addr_1];
       if(!pde->present) {
       ...
       if(!pde->user_supervisor || !pde->read_write) {
          mmu_raise_segmentation_fault(this);
          return;
       }

       page_table *pt = (page_table *)get_system_pointer_from_pde(pde);
       result = &pt->entries[addr_2];

       if(!result->present) {
          mmu_raise_page_fault(this);
          to_physical((page_table_entry*) result);
       }

       if(!result->user_supervisor || !result->read_write) {
          mmu_raise_segmentation_fault(this);
          return;
       }

       result->accessed = 1;
       tlb_add_pte(&this->tlb, virtual_address >> 12, result);
    }
    else {
       if(!result->present) {
          mmu_raise_page_fault(this);
    ...
    addr32 f = (result->base_addr << 12) + offset;
    memcpy(buffer, get_system_pointer_from_address(f), num_bytes);
}
...
void mmu_write_to_virtual_address(mmu *this, addr32 virtual_address, size_t pid,
    ...
    addr32 f = (result->base_addr << 12) + offset;
    memcpy(get_system_pointer_from_address(f), buffer, num_bytes);
}
