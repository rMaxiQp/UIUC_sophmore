...
void TileImage::paste(PNG& canvas, int startX, int startY, int resolution) {
    ...
    if((unsigned)resolution == img_.width())
    {
      for (int x = 0; x < resolution*resolution; x++)
      {
        *(canvas.getPixel(startX + x%resolution,startY + x/resolution))
        = *(img_.getPixel(x%resolution,x/resolution));
      }
      return;
    }
    img_ = PNG(resolution, resolution);
    if (getResolution() % resolution == 0) {
        ...
        for (int x = 0; x < resolution; x++) {
            for (int y = 0; y < resolution; y++) {
                ...
                *(img_.getPixel(x,y)) = *(canvas.getPixel(startX + x, startY + y));
            }
        }
