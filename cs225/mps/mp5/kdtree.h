...
class KDTree
{
  ...
  private:
    ...
    void buildKD(int left, int right, int dimension);

    /** Helper function for constructor*/
    void select(int left, int right, int median, int dimension);

    /** Helper function for constructor*/
    int partition(int left, int right, int median, int dimension);
    ...
    void find(int left, int right, int K, Point<Dim>& current, const Point<Dim>& target) const;

    double calculateDistance(const Point<Dim>& first, const Point<Dim>& second) const
    {
      double distance = 0.0;
    ...
    bool inRange(const Point<Dim>& target, const Point<Dim>& current, int K, double radius) const
    {
      double distance = (target[K] - current[K]) * (target[K] - current[K]);
