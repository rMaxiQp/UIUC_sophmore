...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     {
       TreeNode* smallLeft = removeSmallest(singleQueue, mergeQueue);
       TreeNode* smallRight = removeSmallest(singleQueue, mergeQueue);
       int freq = smallLeft->freq.getFrequency() + smallRight->freq.getFrequency();
       TreeNode* newNode = new TreeNode(freq);
       newNode->left = smallLeft;
       newNode->right = smallRight;
