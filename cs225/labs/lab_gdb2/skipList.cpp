...
void SkipList::insert(int key, HSLAPixel value)
{
    ...
    // expand head and tail to encompassdePointers[level].next->key; the new height of the list, if needed
...
/**findRHelper(int key, int level, SkipNode * curr  int nextKey = traverse->nodePointers[level].next->key;

        if (nextKey == key)
        {
            retNode = traverse->nodePointers[level].ne)
