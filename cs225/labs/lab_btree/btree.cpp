...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
     ...
     * E.g.
     * | 2 | 4 |
     * Mid element is at index (2 - 1) / 2 = 0 .mid_elem_itr
 ...
 * write an equivalent s!subroot->elements.empty() && eemed more instructive.
 */
template <class K, class V>
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    else subroot->elements.insert(subroot->elements.begin() + first_larger_idx, pair);
}
